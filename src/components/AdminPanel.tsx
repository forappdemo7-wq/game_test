...
import { Shield, Server, Activity, Users, Settings, Plus, RotateCcw, X } from 'lucide-react';
...
export const AdminPanel: React.FC<AdminPanelProps> = ({ onClose, localUserId }) => {
  ...
  // Config variables (that can be adjusted)
  ...
  const fetchAdminData = async () => {
    try {
      ...
      // Fetch users catalog for management
      const leadRes = await fetch('/api/leaderboards');
      if (leadRes.ok) {
        const leadData = await leadRes.json();
        // Extract all players combined
        setUsersList(leadData.global || []);
      }

      // Simulated matches logs in store
      const clansRes = await fetch('/api/clans');
      // match history is embedded or managed via files. We'll populate dummy match logs if missing.
    } catch (e) {
  ...
  useEffect(() => {
    fetchAdminData();
    const interval = setInterval(fetchAdminData, 4000); // Poll server every 4s
    return () => clearInterval(interval);
  }, []);

  // Admin adjustments operations
  const grantBonus = async (targetId: string, coins: number, xp: number) => {
    try {
      // Hit shop endpoints which handles additions or custom debug endpoints
      await fetch('/api/shop/buy', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ userId: targetId, cosmeticId: 'grant_premium_bonus_debug' }),
      }).then(() => {
        // Fallback local memory boost simulation if debug code needs updating
      });

      // Instead of relying on specific endpoints, we simulate localized feedback in this demo if desired,
      // but let's notify.
            ...
            <div className="md:col-span-2 bg-[#0b0e27] border border-[#151c3f] p-4 rounded-xl space-y-3">
              ...
              <div className="space-y-2 max-h-64 overflow-y-auto font-mono text-2xs">
                {usersList.map((user) => (
                  ...
                  >
                    <div>
                      <h5 className="font-extrabold text-sm font-sans text-slate-100">{user.username}</h5>
                      <span className="block text-[#707bb1] text-3xs mt-0.5">
                        Role: {user.role} • Coins: 🪙 {user.coins} • Level: {user.level} (RP: {user.rankPoints})
                      </span>
                    </div>

                    <div className="flex gap-1.5 shrink-0">
                      <button
                        onClick={() => grantBonus(user.id, 1000, 200)}
